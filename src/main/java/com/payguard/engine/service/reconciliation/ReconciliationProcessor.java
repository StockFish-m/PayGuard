...
public class ReconciliationProcessor {
        ...
        public void processRow(String orderCode, long payOsAmount, TransactionStatus dbStatus, long dbAmount) {
                ...
                if ((dbStatus == TransactionStatus.PENDING || dbStatus == TransactionStatus.FAILED)
                                && payOsAmount == dbAmount) {
                        log.warn("==> [Reconciliation] Order {} needs to be updated to SUCCESS. currentStatus={}, payOSAmount={}, dbAmount={}.",
                                        orderCode, dbStatus, payOsAmount, dbAmount);

                        // Dùng State Machine kiểm tra xem trạng thái hiện tại có được phép chuyển sang
                        // SUCCESS qua event PAYMENT_SUCCESS không
                        ...
                        TransactionStatus newStatus = stateMachine.transition(dbStatus,
                ...
                if (payOsAmount != dbAmount) {
                        ...
                        // Dùng State Machine kiểm tra xem trạng thái hiện tại có được phép chuyển sang
                        // AMOUNT_MISMATCH qua event PAYMENT_AMOUNT_MISMATCH không
